# A Story Repo QA Review

Run: `2026-06-07_17-57_auto`
Overall status: `blocked`

## Blocking Findings

- `agent_assignment_gate` / `AGENT_ASSIGNMENT_MISSING`: Agent assignment proof is incomplete before prompt lock or imagegen.
- `local_identity_execution_report` / `LOCAL_CPU_EXECUTION_STALLED`: Local identity execution report records CPU stall and blocks final art.

## Reference Summary

- `aachu_face_identity_count`: `4`
- `zuv_face_identity_count`: `4`
- `expression_support_count`: `0`
- `together_body_language_count`: `2`
- `style_reference_count`: `9`
- `text_and_brand_reference_count`: `4`
- `view_image_queue_count`: `32`
- `view_image_queue_actual_count`: `32`
- `prompt_only_allowed`: `False`

## Checks

- `skill_contract_present`: `pass` / `blocker`
- `master_prompt_present`: `pass` / `blocker`
- `required_templates_present`: `pass` / `blocker`
- `reference_manifest_integrity`: `pass` / `blocker`
- `memory_recall`: `not_applicable` / `info`
- `prompt_story_contract`: `not_applicable` / `info`
- `prompt_reference_gate`: `not_applicable` / `info`
- `prompt_forbidden_role_reversal`: `not_applicable` / `info`
- `reference_visibility_proof`: `not_applicable` / `info`
- `image_qa_blocks_final_package`: `pass` / `blocker` (`IMAGE_QA_FAILED`)
- `final_package_not_started`: `pass` / `blocker`
- `accepted_candidate_filename_guard`: `not_applicable` / `info`
- `trace_jsonl_valid`: `pass` / `blocker`
- `hitl_order_before_imagegen`: `pass` / `blocker`
- `agent_assignment_gate`: `fail` / `blocker` (`AGENT_ASSIGNMENT_MISSING`)
- `approvals_cover_required_gates`: `pass` / `blocker`
- `local_identity_execution_report`: `pass` / `blocker` (`LOCAL_CPU_EXECUTION_STALLED`)
